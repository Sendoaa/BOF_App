 ...
 * La Clase Equipos.
 */
public class Jugadores extends JFrame implements ActionListener, WindowListener {
	...
	private DefaultListModel<Jugador> elm;
	...
	public Jugadores() {
		...
		btnJugadores.setBackground(new Color(184, 207, 229));
		...
		elm = new DefaultListModel<Jugador>();

		// JList<Jugador> es = new JList<Jugador>();
		lstJugadores = new JList<Jugador>();
		lstJugadores.setBackground(new Color(227, 227, 227));
		lstJugadores.setFont(new Font("Dialog", Font.BOLD, 20));
		lstJugadores.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		lstJugadores.setBounds(131, 55, 612, 321);
		/* lstTemporada.addListSelectionListener(this); */
		// asocio el DefaultListModel a la lista
		lstJugadores.setModel(elm);
		panelCentral.add(lstJugadores);

		// Crear JScrollPane y agregar la lista a él
		scrollPane = new JScrollPane(lstJugadores);
		...
		if (Seleccion.getTemporadaSeleccionada() != null) {
			...
			try {
				...
				Integer temporadaNum = Seleccion.getTemporadaNumero();
				ResultSet rs = st.executeQuery("SELECT * FROM jugador WHERE DNIJug IN (SELECT DNIJug FROM pertenece WHERE NumTemp = "+temporadaNum+");");
				...
				public void valueChanged(ListSelectionEvent e) {
					if (!e.getValueIsAdjusting()) {
						...
						if (lstJugadores.getSelectedValue() != null) {
							//Guardar el jugador seleccionado en la variable estática
							Seleccion.jugadorSeleccionado = lstJugadores.getSelectedValue();
	...
	private void MostrarJugador() {
		...
		MJ.EnviarJugador(elm.getElementAt(selectedIndex));
		MJ.setVisible(true);
		setEnabled(false);
		// Centrar la ventana en el centro de la pantalla
