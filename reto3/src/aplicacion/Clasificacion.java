...
public class Clasificacion extends JFrame implements ActionListener, WindowListener {
	...
	public Clasificacion() {
		...
		btnTemporadas.setBounds(0, 0, 158, 85);
		...
		btnClasificacion.setBounds(0, 85, 158, 85);
		...
		btnJornadas.setBounds(0, 169, 158, 85);
		panelLateral.add(btnJornadas);

		// Crea un botón
		btnJugadores = new JButton("Jugadores");
		btnJugadores.setFont(new Font("Dialog", Font.BOLD, 15));
		...
		btnJugadores.setBackground(new Color(20, 185, 255));
		btnJugadores.setBounds(0, 339, 158, 85);
		panelLateral.add(btnJugadores);
		...
		btnEquipos.setBounds(0, 254, 158, 85);
	...
	private void toXML() {
		...
		File clubesFolder = new File("ficheros/clubes");
		File xmlFolder;
		...
		if (!basePath.exists()) {
			...
			if (userSelection == JFileChooser.APPROVE_OPTION) {
				...
				xmlFolder = new File(folderToSave, "XML_temporada" + Seleccion.getTemporadaNumero());
				if (!xmlFolder.exists()) {
					xmlFolder.mkdirs();
				}

				// Obtener la lista de nombres de los archivos de imagen de escudos
				escudosFolder = new File("ficheros/escudos");
				File[] escudosFiles = escudosFolder.listFiles();
				List<String> escudosNombres = new ArrayList<>();
				if (escudosFiles != null) {
					for (File escudoFile : escudosFiles) {
						if (escudoFile.isFile()) {
							escudosNombres.add(escudoFile.getName());
						}
					}
				}

				// Obtener la lista de nombres de los archivos de imagen de escudos
				clubesFolder = new File("ficheros/clubes");
				File[] clubesFiles = clubesFolder.listFiles();
				List<String> clubesNombres = new ArrayList<>();
				if (clubesFiles != null) {
					for (File clubFiles : clubesFiles) {
						if (clubFiles.isFile()) {
							clubesNombres.add(clubFiles.getName());
						}
					}
				}
			} else {
		...
		try (PrintWriter writer = new PrintWriter(calendarioFile)) {
			...
			for (Jornada jornada : ListaJornadas) {
				...
				for (Partido partido : jornada.getListaPartidos()) {
					...
					String extension1 = obtenerExtension(partido.getEquipoLocal().getEscudo());

					writer.println("					<imagen>../xml/XML_temporada" + Seleccion.getTemporadaNumero()
							+ "/escudos/" + partido.getEquipoLocal().getNombre() + extension1 + "</imagen>");
					...
					writer.println("					<enlace>../html/paginaclub/Pag" + partido.getEquipoVisitante().getNombre()
							+ ".html</enlace>");
					String extension2 = obtenerExtension(partido.getEquipoVisitante().getEscudo());
					writer.println("					<imagen>../xml/XML_temporada" + Seleccion.getTemporadaNumero()
							+ "/escudos/" + partido.getEquipoVisitante().getNombre() + extension2 + "</imagen>");
		...
		try (PrintWriter writer = new PrintWriter(clubesFile)) {
			...
			for (Equipo equipo : ListaEquipos) {
				if (!equipo.getNombre().equals("Equipo para Descansar")) {
					...
					writer.println("      <imagen>../xml/XML_temporada" + Seleccion.getTemporadaNumero() + "/escudos/"
							+ equipo.getNombre() + extension + "</imagen>");
					writer.println("    </escudo>");
		...
		File clubFolder = new File(xmlFolder, "clubes");
		if (!clubFolder.exists()) {
			clubFolder.mkdirs();
		}

		for (Equipo equipo : ListaEquipos) {
			File clubFile = new File(clubFolder, "PaginaClub_" + equipo.getNombre() + ".xml");

			if (!equipo.getNombre().equals("Equipo para Descansar")) {
				try (PrintWriter writer = new PrintWriter(clubFile)) {
					// Convertir la tabla de clasificación a XML
					writer.println("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
					writer.println("<?xml-stylesheet type=\"text/xsl\"?>");
					writer.println("<Paginaclubes>");
					writer.println("	<club>");
					writer.println("		<imagen>../xml/XML_temporada" + Seleccion.getTemporadaNumero() + "/escudos/"
							+ equipo.getNombre() + ".png</imagen>");
					writer.println("		<nombre>" + equipo.getNombre() + "</nombre>");
					...
					for (Jugador jugador : equipo.getListaJugadores()) {
						...
						writer.println("				<posicion>" + jugador.getPosicion() + "</posicion>");
		...
		// Crear una carpeta dentro de xmlFolder para los escudos de la temporada
		File escudosTemporadaFolder = new File(xmlFolder, "escudos");
		if (!escudosTemporadaFolder.exists()) {
			escudosTemporadaFolder.mkdirs();
		}
		// Copiar solo los archivos de escudos de los equipos de la temporada
		for (Equipo equipo : ListaEquipos) {
			if (equipo.getNombre().equals("Equipo para Descansar")) {
				return;
			}
			...
			File destinoArchivo = new File(escudosTemporadaFolder, nombreEscudo);
			try {
