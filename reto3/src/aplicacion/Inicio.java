...
public class Inicio extends JFrame implements ActionListener, WindowListener, ListSelectionListener {
	...
	public Inicio() {
		...
		JList<Temporada> ListaTemporadas = new JList<>();
		tlm = new DefaultListModel<>();
		//DefaultListModel<Temporada> tlm = new DefaultListModel<>();
		ListaTemporadas.setModel(tlm);
		...
		try {
			...
			while (rs.next()) {
				String numeroTemp = rs.getString("NumTemp");
				Integer numeroTempInt = Integer.parseInt(numeroTemp);
				String fechaInicio = rs.getString("FechaInicio");
				Fecha fechaInicioFecha = new Fecha(fechaInicio);
				// String fechaFin = rs.getString("FechaFin");
				String estado = rs.getString("estado");
				Temporada temporada = new Temporada(numeroTempInt, fechaInicioFecha, estado);
				tlm.addElement(temporada);
				//lstTemporada.setSelectedIndex(0);
			}
			// Cerrar la conexión
			conexion.close();
		} catch (Exception e) {
		...
		if (Seleccion.getTemporadaSeleccionada() == null && !tlm.isEmpty()
				&& tlm.firstElement().getEstado().equals("Iniciada")) {
		...
		if (lstTemporada.getSelectedIndex() == -1) { // No hay nada seleccionado
		    if (lstTemporada.getModel().getSize() > 0) { // Hay al menos un elemento en la lista
		        lstTemporada.setSelectedIndex(0); // Selecciona el primer elemento
		    }
		}
	}
	...
	private void CerrarSesion() {
		...
		switch (opcion) {
			...
			case JOptionPane.YES_OPTION:
				...
				Logger movimiento = new Logger(Sesion.getUsuarioActual(), "Ha cerrado sesión.", fechaActual,
						horaActual);
