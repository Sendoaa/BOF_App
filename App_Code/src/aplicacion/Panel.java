		...
		if (indiceSeleccionado != -1) {
			...
			if (Sesion.getUsuarioActual().equals(ulm.elementAt(indiceSeleccionado))) {
				...
				Logger movimiento = new Logger(usuariocreado,
						"Ha cambiado los permisos del Usuario " + ulm.elementAt(indiceSeleccionado).getNombre() + ".",
				...
				// Actualizo la lista en el DefaultListModel
		...
		if (indiceSeleccionado != -1) {
			...
			if (opcion == JOptionPane.YES_OPTION) {
				...
				Logger movimiento = new Logger(usuario,
						"Ha eliminado el Usuario " + ulm.elementAt(indiceSeleccionado).getNombre() + ".", fechaActual,
						horaActual);
				...
				if (Sesion.getUsuarioActual().equals(ulm.elementAt(indiceSeleccionado))) {
					...
					ListaUsuarios.remove(indiceSeleccionado);
		...
		if (indiceSeleccionado != -1) {
			...
			if (opcion == JOptionPane.YES_OPTION) {
				...
				if (!PerteneceTemporada) {
					// Elimina el archivo del escudo
					File archivoEscudo = new File(elm.getElementAt(indiceSeleccionado).getEscudo());
					if (archivoEscudo.exists()) {
						archivoEscudo.delete();
					}
				}
